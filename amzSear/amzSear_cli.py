from consts import *
from amzSear import getItem,getSearchPage,getCleanPrices,getRatingValue

import sys
import webbrowser

def getKwargs(args):
	options = {'q':False,'d':True,'a':False}
	ignore = []
	...
	translate = {'i':'item_number','p':'page_number',
				 'q':'quiet','d':'open_url','a':'print_all_info'}
	out = {}
...
def printProducts(products,print_all_info=False):
	...
	for count in sorted(products.keys()):
		if print_all_info:
			...
			star_rating = '*' * round(getRatingValue(products[count]['rating']))
...
if __name__ == '__main__':
	if len(sys.argv) < 2 or sys.argv[1].startswith('-'):
		print("Usage: %s query_string [-p num [-i num]] [-q] [-d] [-a]")
		exit(1)
